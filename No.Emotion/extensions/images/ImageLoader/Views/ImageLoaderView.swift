...
/// Loads the image using provided URLRequest while displaying the placeholder view or the content view for incremental
/// loading
...
struct ImageLoaderView<Content, Placeholder>: View where Content: View, Placeholder: View {
    struct Properties {
        ...
        var imageDownloadHandlerProperties: ImageDownloadHandler.Properties {
            ...
            return ImageDownloadHandler.Properties(
    ...
    var body: some View {
        ...
        let partialCallback: ImageDownloadHandler.PartialCallback = { imageFrames in
            ...
            if imageFrames.count == 1 {
                viewModel.imageProxy = ImageWrapper(
                    cgImage: imageFrames.first!.image,
                    cgOrientation: imageFrames.first!.orientation
                )
            } else {
        ...
        let handler = ImageDownloadHandler(
            properties: properties.imageDownloadHandlerProperties,
        ...
        return ImageLoaderContentView(model: viewModel, placeholder: placeholder, content: content)
            .onAppear {
                self.services.downloadService.add(
                    handler,
                    forURLRequest: self.properties.urlRequest,
                    withFileIdentifier: self.properties.fileIdentifier
                )
                self.services.downloadService.load(
                    urlRequest: self.properties.urlRequest,
                    withFileIdentifier: self.properties.fileIdentifier,
                    after: self.properties.delay,
                    expiryDate: self.properties.expiryDate
                )
            }
            .onDisappear {
                self.services.downloadService.remove(
                    handler,
                    fromURLRequest: self.properties.urlRequest,
                    withFileIdentifier: self.properties.fileIdentifier
                )
            }
    }
